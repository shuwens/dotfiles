...
function query_problem()
{
    ...
    QUESTION_CONTENT=$(xidel -s ${TMP_JSON_FILE} -e '$json("data").question.content' | sed -e 's/<[^>]*>//g')

    QUESTION_DIFFICULTY=$(xidel -s ${TMP_JSON_FILE} -e '$json("data").question.difficulty')

    QUESTION_TITLE=$(xidel -s ${TMP_JSON_FILE} -e '$json("data").question.questionTitle')

    QUESTION_ID=$(xidel -s ${TMP_JSON_FILE} -e '$json("data").question.questionId')

    QUESTION_CATEGORY=$(xidel -s ${TMP_JSON_FILE} -e '$json("data").question.categoryTitle')
